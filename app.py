...
@app.route('/generar-utm')
def generar_utm():
    return render_template('utm.html')

@app.route('/utm-generada', methods=['POST'])
def utm_generada():
    utm_link = request.form['utm_link']
    utm_campaign = request.form['utm_campaign']
    utm_source = request.form['utm_source']
    utm_medium = request.form['utm_medium']
    utm_term = request.form['utm_term']
    utm_content = request.form['utm_content']
    
    url_completa = f'{utm_link}?utm_campaign={utm_campaign}&utm_source={utm_source}&utm_medium={utm_medium}&utm_term={utm_term}&utm_content={utm_content}'
    return render_template('resultado.html', url_completa=url_completa)
